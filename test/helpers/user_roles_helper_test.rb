...
class UserRolesHelperTest < ActionView::TestCase
  ...
  def test_role_icon_administrator
    ...
    create(:user) do |user|
      ...
      icon_dom = Rails::Dom::Testing.html_document_fragment.parse(icon)
      assert_dom icon_dom, "a:root[href='#{user_role_path(user, 'moderator')}'][data-method='put']", :count => 1 do
      ...
      icon_dom = Rails::Dom::Testing.html_document_fragment.parse(icon)
      assert_dom icon_dom, "a:root[href='#{user_role_path(user, 'importer')}'][data-method='put']", :count => 1 do
        assert_dom "> svg", :count => 1 do
  ...
  def test_role_icons_administrator
    ...
    create(:user) do |user|
      ...
      assert_dom icons_dom, "a:root[href='#{user_role_path(user, 'administrator')}'][data-method='put']" do
        assert_dom "> svg > title", :text => "Grant administrator access"
      end
      assert_dom icons_dom, "a:root[href='#{user_role_path(user, 'moderator')}'][data-method='put']" do
        assert_dom "> svg > title", :text => "Grant moderator access"
      end
      assert_dom icons_dom, "a:root[href='#{user_role_path(user, 'importer')}'][data-method='put']" do
