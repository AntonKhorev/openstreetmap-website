...
module Searches
  class OsmNominatimReverseQueriesControllerTest < QueriesControllerTest
    ##
    # test all routes which lead to this controller
    def test_routes
      assert_routing(
        { :path => "/search/osm_nominatim_reverse_query", :method => :post },
        { :controller => "searches/osm_nominatim_reverse_queries", :action => "create" }
      )
    end

    def test_create
      with_http_stubs "nominatim" do
        post search_osm_nominatim_reverse_query_path(:lat => 51.7632, :lon => -0.0076, :zoom => 15), :xhr => true
        ...
        post search_osm_nominatim_reverse_query_path(:lat => 51.7632, :lon => -0.0076, :zoom => 17), :xhr => true
        ...
        post search_osm_nominatim_reverse_query_path(:lat => 13.7709, :lon => 100.50507, :zoom => 19), :xhr => true
