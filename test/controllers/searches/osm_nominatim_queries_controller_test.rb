...
module Searches
  class OsmNominatimQueriesControllerTest < QueriesControllerTest
    ##
    # test all routes which lead to this controller
    def test_routes
      assert_routing(
        { :path => "/search/osm_nominatim_query", :method => :post },
        { :controller => "searches/osm_nominatim_queries", :action => "create" }
      )
    end

    def test_create
      with_http_stubs "nominatim" do
        post search_osm_nominatim_query_path(:query => "Hoddesdon", :zoom => 10,
        ...
        post search_osm_nominatim_query_path(:query => "Broxbourne", :zoom => 10,
                                             :minlon => -0.559, :minlat => 51.217,
                                             :maxlon => 0.836, :maxlat => 51.766), :xhr => true
