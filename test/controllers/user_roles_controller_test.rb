...
class UserRolesControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_routes
    assert_routing(
      { :path => "/user/username/roles/rolename", :method => :put },
      { :controller => "user_roles", :action => "update", :user_display_name => "username", :role => "rolename" }
    )
    assert_routing(
  ...
  def test_update
    ...
    put user_role_path(target_user, "moderator")
    ...
    UserRole::ALL_ROLES.each do |role|
      ...
      assert_difference "UserRole.count", 0 do
        put user_role_path("non_existent_user", role)
      end
      ...
      assert_no_difference "UserRole.count" do
        put user_role_path(super_user, role)
      end
      ...
      assert_difference "UserRole.count", 1 do
        put user_role_path(target_user, role)
      end
    ...
    assert_difference "UserRole.count", 0 do
      put user_role_path(target_user, "no_such_role")
    end
    assert_redirected_to user_path(target_user)
