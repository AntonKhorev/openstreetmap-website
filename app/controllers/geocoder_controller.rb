class GeocoderController < ApplicationController
  ...
  def search
    ...
    if params[:lat] && params[:lon]
      ...
      @sources.push(:name => "osm_nominatim_reverse", :url => osm_nominatim_reverse_query_url(:format => "html"),
                    :fetch_url => search_osm_nominatim_reverse_query_path(params.permit(:lat, :lon, :zoom)))
    elsif params[:query]
      @sources.push(:name => "osm_nominatim", :url => osm_nominatim_query_url(:format => "html"),
                    :fetch_url => search_osm_nominatim_query_path(params.permit(:query, :minlat, :minlon, :maxlat, :maxlon)))
    end
